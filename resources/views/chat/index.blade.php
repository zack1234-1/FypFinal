...
<div class="container-fluid h-100">
        ...
        <div class="col-md-8 col-lg-9 p-0 d-flex flex-column">
            @if ($toId)
                               ...
                               @php
                                ...
                                @if ($isFileReply)
                                    <div class="mb-2">
                                        ...
                                        <button onclick="downloadStoredFile('{{ $msg->reply_file_name }}')"
                                ...
                                <div class="text-end small text-muted">
                                    {{ $msg->created_at->timezone('Asia/Kuala_Lumpur')->format('h:i A') }}
                                </div>
                                                ...
                                                <form onsubmit="submitReply(event, '{{ $msg->id }}')" enctype="multipart/form-data">
                                                    ...
                                                    <div class="row g-2 align-items-center">
                                                        <div class="col-12 col-md-8">
                                                            <input type="text" name="reply_message" class="form-control" placeholder="Type your reply...">
                                                        </div>
                ...
                <!-- Message Input -->
                <div class="position-fixed bottom-0 " style="left: 640px; width: 54%; z-index: 100;">
...
<div class="modal fade" id="forwardModal" tabindex="-1" aria-labelledby="forwardModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form id="forwardForm" method="POST" action="{{ route('chat.forward') }}">
      ...
      <input type="hidden" name="attachment" id="forwardMessageAttachment">
      <input type="hidden" name="file_name" id="forwardMessageFileName">
      <div class="modal-content">
        <div class="modal-header">
...
<script>
    ...
    async function sendMessage(e) {
        ...
        if (toId) formData.append('to_id', toId);  
    ...
    async function editMessage(el, id) 
    {
      ...
      document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('[data-bs-target^="#replyModal-"]').forEach(el => {
            ...
            const rawFileName = button.getAttribute('data-file-name');

            document.getElementById('forwardMessageId').value = messageId;
            document.getElementById('forwardMessageBody').value = messageBody;
            document.getElementById('forwardMessageFileName').value = rawFileName;
            ...
            if (attachmentUrl) {
                try {
                    ...
                    fileName = rawFileName || attachmentUrl.split('/').pop(); // ✅ fallback only if no filename passed
                } catch (error) {
        ...
        forwardForm.addEventListener('submit', function (e) {
            ...
            formData.append('original_message_id', document.getElementById('forwardMessageId').value);
            formData.append('body', document.getElementById('forwardMessageBody').value);
            formData.append('to_id', document.getElementById('forward_to_id').value);
            formData.append('group_id', document.getElementById('forward_group_id').value);
            ...
            .then(async response => {
                if (!response.ok) {
                    ...
                    alert('Forward failed: ' + (error.message || 'Validation error'));
                    return;
                }
            ...
            .then(data => {
                if (data?.redirect) {
                    ...
                    alert('Message forwarded, but no redirect URL provided.');
                }
            })
            .catch(error => {
